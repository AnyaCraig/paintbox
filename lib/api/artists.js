...
router.post('/:artist_id', async (req, res, next) => {
    ...
    try {
        const artworkDoc = await artwork.save();
        ...
        doc.artworks.push(artworkDoc._id);
...
router.delete('/:artist_id', async (req, res, next) => {
    try {
      const { artist_id } = req.params
      const doc = await Artist.findByIdAndRemove(artist_id)
...
router.patch('/:artist_id', async (req, res, next) => {
    try {
      const { artist_id } = req.params
      const doc = await Artist.findByIdAndUpdate(artist_id, req.body);
      res.status(200).send(doc)
    } catch (e) {
      next(e)
    }
